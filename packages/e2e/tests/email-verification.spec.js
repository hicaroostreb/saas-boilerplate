...
test.describe('Email Verification Tests', () => {
  test('should validate email format during registration', async ({ page }) => {
    ...
    for (const invalidEmail of invalidEmails) {
      ...
      const isInvalid = await emailField.evaluate(el => !el.validity.valid) ||
                       await page.locator('input[name="email"][aria-invalid="true"]').isVisible() ||
                       await page.locator('text=/invalid.*email|valid.*email.*address/i').isVisible();
      ...
      if (!isInvalid) {
        ...
        const submitError = await page.locator('text=/invalid.*email|valid.*email/i').isVisible();
  ...
  test('should handle email verification flow after registration', async ({ page }) => {
    ...
    const verificationMessage = await page.locator('text=/verify.*email|check.*email|sent.*email/i').isVisible();
    const verificationPage = currentUrl.includes('verify') || currentUrl.includes('confirm');
    ...
    if (verificationMessage || verificationPage) {
      ...
      if (verificationPage) {
        ...
        if (await resendButton.isVisible()) {
          ...
          const resendConfirm = await page.locator('text=/sent|resent|check/i').isVisible();
  ...
  test('should prevent duplicate email registration', async ({ page }) => {
    ...
    const duplicateError = await page.locator('text=/already.*exist|email.*taken|already.*registered/i').isVisible();
    const staysOnPage = page.url().includes('sign-up');

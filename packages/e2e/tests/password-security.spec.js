...
test.describe('Password Security Tests', () => {
  ...
  test('should reject weak passwords', async ({ page }) => {
    ...
    for (const weakPassword of weakPasswords) {
      ...
      const hasWeakPasswordError = await page.locator('text=/weak|strong|must contain|at least/i').isVisible() ||
      ...
      const submitError = await page.locator('text=/weak|strong|password.*require/i').isVisible();
      const staysOnPage = page.url().includes('sign-up');
      
      console.log(`Password "${weakPassword}": error=${submitError}, stays=${staysOnPage}`);
  ...
  test('should accept strong passwords', async ({ page }) => {
    ...
    for (const strongPassword of strongPasswords) {
      ...
      const hasError = await page.locator('text=/weak|too simple|not strong/i').isVisible();
      expect(hasError).toBeFalsy();
      
      // Check if field is marked as valid (if validation exists)
      const isValid = await passwordField.evaluate(el => !el.validity || el.validity.valid);
  ...
  test('should validate password confirmation matching', async ({ page }) => {
    ...
    const hasMismatchError = await page.locator('text=/match|confirm|same/i').isVisible() ||
                            await page.locator('input[name="confirmPassword"][aria-invalid="true"]').isVisible();
    ...
    if (!hasMismatchError) {
      ...
      const submitError = await page.locator('text=/match|confirm|same/i').isVisible();
      const staysOnPage = page.url().includes('sign-up');

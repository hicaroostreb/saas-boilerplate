...
const getDeviceIcon = (deviceType: SessionDevice['deviceType']) => {
  switch (deviceType) {
    ...
    case 'tablet':
      return (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M6 2a2 2 0 00-2 2v12a2 2 0 002 2h8a2 2 0 002-2V4a2 2 0 00-2-2H6zM5 4a1 1 0 011-1h8a1 1 0 011 1v12a1 1 0 01-1 1H6a1 1 0 01-1-1V4z" clipRule="evenodd" />
        </svg>
      );
    case 'desktop':
      return (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M3 5a2 2 0 012-2h10a2 2 0 012 2v8a2 2 0 01-2 2h-4l1.5 2H14a1 1 0 110 2H6a1 1 0 110-2h1.5L9 15H5a2 2 0 01-2-2V5zm2-1a1 1 0 00-1 1v8a1 1 0 001 1h10a1 1 0 001-1V5a1 1 0 00-1-1H5z" clipRule="evenodd" />
        </svg>
      );
    default:
      return (
...
const getRiskLevelColor = (riskScore: number) => {
  if (riskScore >= 80) return { bg: 'bg-red-100', text: 'text-red-700', dot: 'bg-red-500' };
  if (riskScore >= 60) return { bg: 'bg-orange-100', text: 'text-orange-700', dot: 'bg-orange-500' };
  if (riskScore >= 30) return { bg: 'bg-yellow-100', text: 'text-yellow-700', dot: 'bg-yellow-500' };
...
export function SessionManager({
  ...
  const renderSessionItem = (session: SessionDevice, isCurrentSession = false) => {
              ...
              <div className="flex items-center space-x-3 mt-2">
                ...
                {showRiskScores && (
                  <div className="flex items-center space-x-1">
                    <div className={cn("w-2 h-2 rounded-full", riskColors.dot)} />
                    <span className="text-xs text-muted-foreground">
    ...
    <div className={cn("space-y-6", className)}>
      ...
      {sessions.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {currentSession && (
            <div>
              <h3 className="text-sm font-medium text-foreground mb-3">This Device</h3>
        ...
        <div className="flex items-start space-x-2">
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            ...
            <ul className="space-y-1">
              ...
              <li>• Use "Revoke All Other Sessions" if you suspect unauthorized access</li>
              {organizationSlug && (
                <li>• These sessions are specific to your {organizationSlug} account</li>
              )}
            </ul>
          </div>
        </div>
          ...
          <div className="bg-background rounded-lg shadow-lg max-w-md w-full p-6">
            <div className="flex items-center space-x-2 mb-4">
              ...
              <h3 className="text-lg font-semibold">
                {showConfirmDialog.type === 'all' ? 'Revoke All Other Sessions?' : 'Revoke Session?'}
              </h3>
            </div>
            
            <p className="text-sm text-muted-foreground mb-6">
              {showConfirmDialog.type === 'all' 
                ? `This will sign you out of all other devices (${otherSessions.length} sessions). You'll need to sign in again on those devices.`
                : `This will sign you out of "${showConfirmDialog.deviceName || 'this device'}". You'll need to sign in again on that device.`
              }
            </p>
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                onClick={() => {
                  if (showConfirmDialog.type === 'all') {
                    ...
                    handleRevokeSession(showConfirmDialog.sessionToken, showConfirmDialog.deviceName);
                  }
                }}
                disabled={isRevokingAll || revokingSessionToken !== null}
              ...
              >
                {(isRevokingAll || revokingSessionToken) ? 'Revoking...' : 'Revoke'}
              </button>
            </div>
          </div>
        </div>
    ...
    <div className={cn("space-y-2", className)}>
      {sessions.map((session) => (
        ...
        >
          ...
          {!session.isCurrent && (
            <button
              onClick={() => onRevokeSession(session.sessionToken)}
    ...
    <div className={cn("p-4 border rounded-lg", className)}>
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <div className={cn("w-2 h-2 rounded-full ml-auto mt-1", riskColors.dot)} />
        </div>
      </div>
    </div>
  );

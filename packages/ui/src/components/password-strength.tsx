...
export function PasswordStrength({ 
  ...
  const { requirements, strengthLevel, strengthScore, strengthColor } = React.useMemo(() => {
    ...
    const hasNoPersonalInfo = !email || !password.toLowerCase().includes(email.split('@')[0].toLowerCase());
    const hasNoNameInfo = !name || !password.toLowerCase().includes(name.toLowerCase());
    ...
    if (password.length >= 4) { // Only show these for longer passwords
      reqs.push({
        id: "patterns",
    ...
    const metWeight = reqs.filter(req => req.met).reduce((sum, req) => sum + req.weight, 0);
    ...
    const colors = {
      weak: { bg: 'bg-red-200', text: 'text-red-700', progress: 'bg-red-500' },
      fair: { bg: 'bg-orange-200', text: 'text-orange-700', progress: 'bg-orange-500' },
      good: { bg: 'bg-yellow-200', text: 'text-yellow-700', progress: 'bg-yellow-500' },
      strong: { bg: 'bg-green-200', text: 'text-green-700', progress: 'bg-green-500' },
    };
  ...
  if (variant === 'minimal' && !showAllRequirements) {
    ...
    return (
      <div className={cn("flex items-center gap-1.5 px-1 text-[0.8rem] font-medium text-muted-foreground", className)}>
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-3.5 w-3.5">
      ...
      <div className={cn("space-y-2", className)}>
        ...
        <div className="space-y-1">
          ...
          <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
            <div 
              className={cn("h-full transition-all duration-300", strengthColor.progress)}
...
export function getPasswordStrength(password: string, options?: {
  requireSpecialChars?: boolean;
  minLength?: number;
  email?: string;
  name?: string;
  ...
  if (password.length < (options?.minLength || 8)) {
    suggestions.push(`Use at least ${options?.minLength || 8} characters`);
  }
  ...
  let score = 0;
  if (password.length >= (options?.minLength || 8)) score += 25;

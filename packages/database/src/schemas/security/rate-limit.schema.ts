...
// ============================================
// RATE LIMITS SCHEMA - ENTERPRISE RATE LIMITING (REFACTORED)
// ============================================
...
export const rate_limits = pgTable(
  'rate_limits',
  {
    ...
    tenant_id: text('tenant_id'), // ✅ ADICIONADO - Nullable para system-level limits
    ...
    window_size: integer('window_size').notNull().default(1),
  ...
  table => ({
    ...
    tenantTypeIdentifierIdx: index('rate_limits_tenant_type_identifier_idx').on(
      table.tenant_id,
      table.type,
      table.identifier
    ),
    tenantOrgIdx: index('rate_limits_tenant_org_idx').on(
      table.tenant_id,
      table.organization_id
    ),

    // Primary access pattern
    typeIdentifierIdx: index('rate_limits_type_identifier_idx').on(
      table.type,
      table.identifier
    ),
...
export type RateLimitWindow =
  (typeof rate_limit_window_enum.enumValues)[number];

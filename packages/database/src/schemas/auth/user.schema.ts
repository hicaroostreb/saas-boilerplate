...
// ============================================
// USER SCHEMA - ENTERPRISE MULTI-TENANT (REFACTORED)
// ============================================
...
export const users = pgTable(
  'users',
  {
    ...
    tenant_id: text('tenant_id').notNull(), // ✅ ADICIONADO - Multi-tenancy obrigatório
    ...
    // ✅ ADICIONADO - Metadata extensível (JSONB)
  ...
  table => ({
    ...
    emailVerifiedIdx: index('users_email_verified_idx').on(
      table.is_email_verified
    ),

    // Security indexes
    lastLoginIdx: index('users_last_login_idx').on(table.last_login_at),
    lockedIdx: index('users_locked_idx').on(table.locked_until),
    loginAttemptsIdx: index('users_login_attempts_idx').on(
...
export function getDisplayName(
  user: Pick<User, 'first_name' | 'name'>
): string {
...
// ✅ NOVO - Parse metadata JSONB
export function parseUserMetadata(user: User): Record<string, any> {
...
export function serializeUserMetadata(metadata: Record<string, any>): string {

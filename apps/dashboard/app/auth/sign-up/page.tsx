...
function SignUpContent() {
  ...
  // ✅ CORREÇÃO CRÍTICA: Detectar erros de redirect
  ...
  const handleCredentialsSubmit = async (data: { 
    ...
    try {
      console.log('🔍 ACHROMATIC: Starting enterprise credentials sign-up');
      ...
      if (result.success) {
        ...
        try {
          ...
          console.error('❌ ACHROMATIC: Sign-in after signup error:', signInError);
          
          // ✅ CORREÇÃO CRÍTICA: Verificar se é erro de redirect
          if (isRedirectError(signInError)) {
            console.log('🔄 Redirect detected after signup, allowing Next.js to handle...');
      ...
      if (isRedirectError(error)) {
        console.log('🔄 Redirect detected in signup, allowing Next.js to handle...');
        // Re-throw para permitir que o Next.js processe o redirect
        throw error;
      }
  ...
  const handleGoogleSubmit = async () => {
    ...
    try {
      console.log('🔍 ACHROMATIC: Starting Google OAuth sign-up');
  ...
  const handleMicrosoftSubmit = async () => {
    ...
    try {
      console.log('🔍 ACHROMATIC: Starting Microsoft OAuth sign-up');
      ...
      if (isRedirectError(error)) {
        console.log('🔄 Microsoft redirect detected, allowing Next.js to handle...');
        throw error;
      }
    ...
    <main className="h-screen dark:bg-background bg-gray-50 px-4">
      <div className="mx-auto w-full min-w-[320px] space-y-6 py-12 max-w-sm">
        ...
        <div className="px-2 text-xs text-muted-foreground space-y-2">
          ...
          {organizationSlug && (
            <p>
              You will be joining <strong>{organizationSlug}</strong> and agree to their organization policies.
            </p>
          )}
    ...
    <Suspense fallback={
      <main className="h-screen dark:bg-background bg-gray-50 px-4">
        <div className="mx-auto w-full min-w-[320px] space-y-6 py-12 max-w-sm">
          ...
          <p className="text-center text-muted-foreground">Loading sign up...</p>
        </div>
      </main>
    }>
      <SignUpContent />
